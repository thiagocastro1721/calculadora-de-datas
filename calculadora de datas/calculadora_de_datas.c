	...
	void captura_e_valida_dados_do_teclado(signed long *pdia_inicial, signed long *pmes_inicial, signed long *pano_inicial, signed long *pdia_final, signed long *pmes_final, signed long *pano_final, signed long *pdias_a_calcular, signed long *popcao)
	{
	    ...
	    else if(*popcao == 7)
	    {
	        ...
	        *pdia_final = 1;
	        ...
	        scanf("%ld/%ld", &*pmes_final, &*pano_final);
	 ...
	 void validar_datas(signed long *pdia_inicial, signed long *pmes_inicial, signed long *pano_inicial, signed long *pdia_final, signed long *pmes_final, signed long *pano_final, signed long *pdias_a_calcular, signed long *popcao)
	 {
	        ...
	        if(*popcao == 1)
	        {
	            ...
	            if((*pano_inicial%4 == 0) && (*pano_inicial%100 != 0) || (*pano_inicial%400 == 0))
	            {
	                if((*pdia_inicial > 29) && (*pmes_inicial == 2))
	                {
	                    //printf("\n*pdia_inicial = %ld, *pdia_final = %ld\n",*pdia_inicial, *pdia_final);
	                    printf("\nData invalida. Verifique os dias. O ano %ld eh bissexto, entao fevereiro tem 29 dias.\n\n", *pano_inicial);
	                    erro++;
	                }
	            }
	            else
	            {
	                ...
	                {
	                    //printf("\n*pdia_inicial = %ld, *pdia_final = %ld\n",*pdia_inicial, *pdia_final);
	                    printf("\nData invalida. Verifique os dias. O ano %ld nao eh bissexto, entao fevereiro tem 28 dias.\n\n", *pano_inicial);
	                    erro++;
	                }
	            }
	...
	int calculadora_de_datas(signed long *pdia_inicial, signed long *pmes_inicial, signed long *pano_inicial, signed long *pdia_final, signed long *pmes_final, signed long *pano_final, signed long *pdias_a_calcular, signed long *popcao)
	{
	    ...
	    else if (*popcao == 1)
	    {
	        ...
	        if(quant_anos > 0)
	        {
	            if(quant_anos == 1)
	            {
	                printf("%ld Ano ", quant_anos);
	            }
	            else
	            {
	                printf("%ld Anos ", quant_anos);
	            }
	        }
	        if(quant_meses > 0)
	        {
	            if(quant_meses == 1)
	            {
	                printf("%ld Mes ", quant_meses);
	            }
	            else
	            {
	                printf("%ld Meses ", quant_meses);
	            }
	        ...
	        if(quant_semanas > 0)
	        {
	            if(quant_semanas == 1)
	            {
	                printf("%ld Semana ", quant_semanas);
	            }
	            else
	            {
	                printf("%ld Semanas ", quant_semanas);
	            }
	        }
	        if(quant_dias > 0)
	        {
	            if(quant_dias == 1)
	            {
	                printf("%ld Dia ", quant_dias);
	            }
	            else
	            {
	                printf("%ld Dias ", quant_dias);
	            }
	        }
	        if(cont_dias > 0)
	        {
	    ...
	    else if (*popcao == 5)
	    {
	        ...
	        if ((*pano_final % 4 == 0) && (*pano_final % 100 != 0) || (*pano_final % 400 == 0))
	        {
	            printf("\nSim. O ano %ld eh bissexto.\n", *pano_final);
	        }
	        else
	        {
	            printf("\nNao. O ano %ld nao eh bissexto.\n", *pano_final);
	        }
	        printf("*******************************************************************************\n");
	        return 0;
	    }
	    else if (*popcao == 6)
	    {
	        if(*pano_final >= *pano_inicial)
	        {
	            ...
	            {
	                 printf("Nao ha anos bissextos  entre %ld e %ld.\n",*pano_inicial, *pano_final);
	            }
	            else
	            {
	                if(contanobissextos == 1)
	                {
	                    printf("Ha %ld ano bissexto entre %ld e %ld. Veja abaixo:\n\n",contanobissextos, *pano_inicial, *pano_final);
	                }
	                else
	                {
	                    printf("Ha %ld anos bissextos entre %ld e %ld. Veja abaixo:\n\n",contanobissextos, *pano_inicial, *pano_final);
	                }
